...
- Soft keepalive every ~15s: ping when online; join once only when fully disconnected (skips Setup AP).
- NM `autoconnect-retries=0` means **forever** (do not use `-1` — that is only the global default of ~4 tries).
