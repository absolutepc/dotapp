...
2. Replace generated sources with files from `ios/Dot/` (including `Views/OnboardingView.swift`, `Views/SettingsView.swift`, `Views/CachedAsyncImage.swift`, `Views/LastSeenLocationView.swift`, `Services/DotLocationTracker.swift`).
...
5. **Настройки** (toolbar gear): brightness slider for the round display, theme, device info, Wi‑Fi wizard / reset to Dot-Setup, clear saved address, re-show intro.
