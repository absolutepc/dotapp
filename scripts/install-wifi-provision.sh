...
# Remove legacy helpers/units (incl. old flap-prone keepalive/burst if present)
systemctl disable --now bmw-wifi-apply.path bmw-wifi-apply.service 2>/dev/null || true
systemctl disable --now dot-wifi-keepalive.timer dot-wifi-keepalive.service 2>/dev/null || true
...
rm -f /etc/systemd/system/dot-wifi-keepalive.timer /etc/systemd/system/dot-wifi-keepalive.service
...
rm -f /usr/local/sbin/dot-wifi-keepalive /usr/local/sbin/dot-wifi-burst /usr/local/sbin/dot-restore-wifi
