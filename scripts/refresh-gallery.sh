...
DATA_ROOT="${DOT_DATA:-/var/lib/dot}"
...
if systemctl is-active dot-api >/dev/null 2>&1; then
  sudo systemctl restart dot-api
  echo "Restarted dot-api"
elif systemctl is-active dot-api >/dev/null 2>&1; then
  sudo systemctl restart dot-api
  echo "Restarted dot-api (legacy name)"
fi
